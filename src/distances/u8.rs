...
pub const METRICS: [(&str, fn(&[u8], &[u8]) -> f32); 4] = [
    ("euclidean", euclidean),
...
#[inline(always)]
pub fn euclidean(x: &[u8], y: &[u8]) -> f32 {
    euclidean_sq(x, y).sqrt()
}

#[inline(always)]
pub fn euclidean_sq(x: &[u8], y: &[u8]) -> f32 {
    x.iter()
        .zip(y.iter())
...
#[inline(always)]
pub fn manhattan(x: &[u8], y: &[u8]) -> f32 {
    x.iter().zip(y.iter()).map(|(&a, &b)| (a as f32 - b as f32).abs()).sum()
}

#[inline(always)]
pub fn cosine(x: &[u8], y: &[u8]) -> f32 {
